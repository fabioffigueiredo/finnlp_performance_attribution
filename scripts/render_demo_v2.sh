...
# Rebuilds the public, captioned LinkedIn demo from a real Playwright capture.
# It never calls an external API or adds an audio track.

project_root="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
media_dir="$project_root/media/generated/2026-W33"
input="finnlp-raw-v2.webm"
output="finnlp-verifiable-demo-v2.mp4"
font_name="Verdana"
...
render_caption 'finnlp-caption-1-v2.png' $'Uma entrada.\nUm caminho inspecionável.'
render_caption 'finnlp-caption-2-v2.png' $'Cenário público\ne fictício.'
render_caption 'finnlp-caption-3-v2.png' $'Resultado do endpoint real.\nHTTP 200.'
render_caption 'finnlp-caption-4-v2.png' $'Interface → endpoint HTTP\n→ serviço.'
render_caption 'finnlp-caption-5-v2.png' $'Projeto acadêmico.\nSem recomendação de investimento.'

ffmpeg -hide_banner -loglevel error -y -i "$input" \
  -loop 1 -i finnlp-caption-1-v2.png \
  -loop 1 -i finnlp-caption-2-v2.png \
  -loop 1 -i finnlp-caption-3-v2.png \
  -loop 1 -i finnlp-caption-4-v2.png \
  -loop 1 -i finnlp-caption-5-v2.png \
  -filter_complex "
    [0:v]trim=start=3:end=6,setpts=PTS-STARTPTS,crop=560:960:220:0,scale=1080:1920:flags=lanczos,setsar=1[screen_a];
    ...
    [0:v]trim=start=25:end=31,setpts=PTS-STARTPTS,crop=560:960:520:0,scale=1080:1920:flags=lanczos,setsar=1[screen_c];
    [3:v]trim=duration=6,setpts=PTS-STARTPTS[caption_c]; [screen_c][caption_c]overlay=0:1515:shortest=1[c];
